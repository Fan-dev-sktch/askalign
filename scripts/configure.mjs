...
const config = { mcpServers: { 'grill-me-ui': { command: process.execPath, args: [server] } } };
...
console.log('codex mcp add grill-me-ui -- ' + quote(process.execPath) + ' ' + quote(server));
console.log('If AskAlign is already installed as a plugin, update that plugin instead; do not register a second server.');
